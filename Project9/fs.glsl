...
{		
	...
	vec3 DirLight1 = DiffColor * DirLightColor*DirLightAmp*max(0.0, dot(normalize(vNormal), -DirLightDir));
	vec3 DirLight2 = 0.8 * DiffColor * DirLightColor*DirLightAmp*max(0.0, dot(normalize(vNormal), DirLightDir));
	
	vec3 halfV = normalize(normalize(CameraPos-mPosition)+normalize(-DirLightDir));
	float NdotHV = max(dot(vNormal,halfV),0.0);
	vec3 DirLightS = DirLightColor * SpecColor * DirLightSpec * 
pow(NdotHV, MaterialShininess);
	//vec4 DirLightSpec = vec4(DirLightS , 1.0);

	vec3 halfV2 = normalize(normalize(CameraPos-mPosition)+normalize(DirLightDir));
	float NdotHV2 = max(dot(vNormal,halfV2),0.0);
	vec3 DirLightS2 = DirLightColor * SpecColor * DirLightSpec * 
pow(NdotHV2, MaterialShininess);
...
* pow(NdotHV, MaterialShininess) * (PntLightFnc[i].x/(0.1*dst*PntLightFnc[i].y));
	}
	...
	vec4 LightSum = vec4(AmbLight + DirLight1 + DirLight2 + DirLightS + DirLightS2 + PntLightSum + PntLightSpecSum, 1.0);
	//LightSum = vec4(DirLightS2+DirLightS, 1.0);
	//LightSum = vec4(DirLight1 + DirLight2, 1.0);
