...
void glObject::LoadFromFile(char *OBJName, char *MTLName, char *ObjectName)
{
	...
	for (int i = 0; i < lF; i++)
	{
		...
		AddVertex(&V[F[i].v1 - 1],0,0);
		SetNormal(&N[F[i].n2 - 1]);
		AddVertex(&V[F[i].v2 - 1], 0, 0);
		SetNormal(&N[F[i].n3 - 1]);
		AddVertex(&V[F[i].v3 - 1], 0, 0);
	}
...
void glObject::FillData(int c, float* v, float* n, float* t){
	this->SetColor(1.0, 1.0, 1.0);
	
	this->BeginObject(GL_TRIANGLES);
	for (int i = 0; i <= c*3-1; i+=3){		
		this->SetNormal(n[i], n[i+1], n[i+2]);
...
//--------------------------------------------------------------------------------------------
void glObject::Draw(glm::mat4 aModelMatrix){
	glUniformMatrix4fv(glProgram::getInstance()->getHandle("normalMatrix"), 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(aModelMatrix))));
	glUniformMatrix4fv(glProgram::getInstance()->getHandle("modelViewMatrix"), 1, GL_FALSE, glm::value_ptr(mCamera->getViewMatrix() * aModelMatrix));
	glUniformMatrix4fv(glProgram::getInstance()->getHandle("ModelMatrix"), 1, GL_FALSE, glm::value_ptr(aModelMatrix));
	this->Draw();
}
