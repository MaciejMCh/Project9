...
LRESULT CALLBACK WndProc(	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM	lParam)
{
	...
	if (uMsg == WM_MOUSEWHEEL){
		...
		if (zDelta < 0)KeyboardController::getInstance()->KeyPress(87);
		else KeyboardController::getInstance()->KeyPress(83);
	}

	if (uMsg == WM_MOUSEMOVE){
		POINT cPos;
		GetCursorPos(&cPos);
		MouseController::getInstance()->Move(cPos.x, cPos.y);
	}

	if (uMsg == WM_LBUTTONDOWN){
		POINT cPos;
		GetCursorPos(&cPos);
		MouseController::getInstance()->LBDown(cPos.x, cPos.y);
	}	

	if (uMsg == WM_LBUTTONUP){
		POINT cPos;
		GetCursorPos(&cPos);
		MouseController::getInstance()->LBUp(cPos.x, cPos.y);
	}

	if (uMsg == WM_RBUTTONDOWN){
		POINT cPos;
		GetCursorPos(&cPos);
		MouseController::getInstance()->RBDown(cPos.x, cPos.y);
	}

	if (uMsg == WM_RBUTTONUP){
		POINT cPos;
		GetCursorPos(&cPos);
		MouseController::getInstance()->RBUp(cPos.x, cPos.y);
	}
